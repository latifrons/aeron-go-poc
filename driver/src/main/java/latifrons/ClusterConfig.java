...
public final class ClusterConfig {
    ...
    public static String udpChannel(final int nodeId, final String hostname, final int portBase, final int portOffset) {
        final int port = calculatePort(nodeId, portBase, portOffset);
        return "aeron:udp?endpoint=" + hostname + ":" + port;
    }

    public static String endpoint(final int nodeId, final String hostname, final int portBase, final int portOffset) {
        return hostname + ":" + calculatePort(nodeId, portBase, portOffset);
    }

    public static IdleStrategy toIdleStrategy(final String idleStrategyName) {
        var vs = idleStrategyName.split(",");

        switch (vs[0]) {
            case "backoffp":
                var v1 = Integer.parseInt(vs[1]);
                var v2 = Integer.parseInt(vs[2]);
                var v3 = Integer.parseInt(vs[3]);
                var v4 = Integer.parseInt(vs[4]);
            ...
            default:
                throw new IllegalArgumentException("Unknown idle strategy: " + idleStrategyName);
        }
    }
}
